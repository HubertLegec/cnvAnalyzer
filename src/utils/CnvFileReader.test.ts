import {CnvFileReader, CnvType} from "./CnvFileReader";
...
test("read file lines", () => {
    ...
    const txtReader = new CnvFileReader(file);
    txtReader.getCnvRows().then(rows => {
        expect(rows).toHaveLength(2);
        expect(rows[0].chromosome).toBe("chr1");
        expect(rows[0].name).toBe("nssv3461613");
        expect(rows[0].type).toBe(CnvType.DELETION);
        expect(rows[1].chromosome).toBe("chr1");
        expect(rows[1].name).toBe("nssv3458820");
        expect(rows[1].type).toBe(CnvType.DELETION);
    });
});